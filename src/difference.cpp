	...
	boost::random::mt19937 random_gen)
{
	...
	hog1->set_param("CELL_GRID_COLS",
		boost::lexical_cast< std::string >(window_size.width / 8));
	hog1->set_param("CELL_GRID_ROWS",
		boost::lexical_cast< std::string >(window_size.height / 8));

	hog2->set_param("CELL_GRID_COLS",
		boost::lexical_cast< std::string >(window_size.width / 8));
	hog2->set_param("CELL_GRID_ROWS",
		boost::lexical_cast< std::string >(window_size.height / 8));
